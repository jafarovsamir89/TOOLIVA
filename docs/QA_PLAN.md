...
Exact Duplicates automated coverage includes unique-size/zero-byte/directory pre-group exclusion, equal-size candidate grouping, SHA-256 equality/inequality, streaming synthetic files, mutation rejection, byte verification, recoverable-byte math, Keep-one selection, last-copy protection and fingerprint-cache reuse/invalidation. The analyzer remains explicit and has no Room/index persistence; its bounded private cache stores only path/size/modified-time/hash metadata.
