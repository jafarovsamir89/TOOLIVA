...
@Composable
private fun VibrationTest(state: HardwareTestsUiState, viewModel: HardwareTestsViewModel) { Column(verticalArrangement = Arrangement.spacedBy(12.dp)) { Text("Press the button for a short vibration. Nothing starts automatically.", color = MaterialTheme.colorScheme.onSurfaceVariant); if (state.hapticFeedbackEnabled == false) Text("System touch vibration is currently disabled. Enable Touch feedback in Android settings before testing.", color = MaterialTheme.colorScheme.secondary); Button(onClick = viewModel::runVibration, Modifier.fillMaxWidth()) { Icon(Icons.Outlined.Vibration, null); Text("Test vibration", Modifier.padding(start = 8.dp)) }; state.errorMessage?.let { Text(it, color = MaterialTheme.colorScheme.error) }; TestDecisionButtons(HardwareTestId.VIBRATION, viewModel) } }
